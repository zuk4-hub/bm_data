...
MIN_LEAD_MIN              = int(os.getenv("MIN_LEAD_MIN", "25"))
...
MIN_EV       = float(os.getenv("MIN_EV", "3.0"))
...
DIAMOND_SLS_THRESHOLD = float(os.getenv("DIAMOND_SLS_THRESHOLD", "97.0"))
...
def fmt_super_game_card(g: Dict[str, Any], header: Optional[Dict[str, Any]] = None) -> List[str]:
    ...
    def _fmt_fair_short(o: Optional[float]) -> str:
        ...
        try:
            ...
            if v <= 0:
                return ""
            # AJUSTE: usar tradução "Odd Justa" em vez de "Fair"
            return f"Odd Justa: {v:0.2f}"
        except Exception:
            return ""
    ...
    if others_with_odds:
        ...
        for m in others_with_odds:
            ...
            # AJUSTE: retirar "Ambas Marcam" ou "Cartões", deixar só a seleção (Sim/Não, Acima/Abaixo)
    ...
    if no_odds:
        ...
        for m in no_odds:
            ...
            # AJUSTE: retirar "Cartões"/"Ambas Marcam", deixar só "Acima de"/"Abaixo de" etc.
            if base and sel and base.lower() in ("ambas marcam", "cartões"):
                display = sel
            elif base and sel:
                display = f"{base} — {sel}"
    ...
    for idx, sec in enumerate(sections):
        ...
        header_lines = _header_full() if idx == 0 else _header_short()
...
def ensure_data_files(force: bool = False) -> None:
    ...
    # ---------------- odds1..odds30.json ----------------
    for i in range(1, 31):
        url = f"{base}/odds{i}.json"
        local_path = f"/data/odds{i}.json"
...
async def _select_by_rule(data: Dict[str, Any], rule: Dict[str, Any], now_local: Optional[datetime] = None):
    ...
    for p in all_picks:
        ...
        ev   = _f(p.get("ev", 0.0))              # EV já vem em %
    ...
    if "singles" in secs:
        for kind, payload, sls, gid in raw_singles:
            ...
            if not _time_ok_lead(_pick_time_str(payload), now_local, MIN_LEAD_MIN):
                continue
        ...
        try:
            ...
            try:
                ...
                if sg:
                    ...
                    for g in games:
                        # ID estável do jogo (pra marcar como já publicado)
                        gid = _super_game_id(g)
                        if _super_pub_is_marked(gid):
                            # já foi mandado hoje, pula
                            continue
                        ...
                        # horário alvo do disparo: SUPER_CARD_LEAD_MIN antes do chute
                        trigger = dt_k - timedelta(minutes=SUPER_CARD_LEAD_MIN)
                        # janela de tolerância (minutos)
                        window  = timedelta(minutes=SUPER_CARD_WINDOW_MIN)

                        # só dispara se estiver dentro da janela [trigger, trigger+window]
                        if not (trigger <= now_local <= trigger + window):
                            continue

                        ok = await _send_super_game_card(g, header=header)
                        if ok:
                            _super_pub_mark(gid)
                            print(f"[SUPER][OK] Enviado Super Game {gid}")
            except Exception as e:
            ...
            try:
                ...
                dt_coruja_utc = datetime.fromisoformat(_to_utc_iso(today_sp, "00:00").replace("Z", "+00:00"))
            ...
            for slot in plan:
                try:
                    ...
                    if ENABLE_FALLBACK_SELECTION:
                        ...
                        try:
                            items = await _select_by_rule(data, rule, now_local=now_local) or []
                        except Exception as e:
